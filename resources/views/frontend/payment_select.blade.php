    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $(".online_payment").click(function() {
                ...
                console.log(clickedValue); // Print the value to the console

                $('#manual_payment_description').parent().addClass('d-none');

                if(clickedValue == "cash_on_delivery")
                {
                    var cod_fee = $('#cod-fee');
                    var fee = cod_fee.attr('data-fee');
                    var total = cod_fee.attr('data-total');
                    var tax = cod_fee.attr('data-tax');

                    // Log the values to the console
                    console.log('Element:', cod_fee);
                    console.log('Fee:', fee);
                    console.log('Total:', total);
                    console.log('Tax:', tax);


                    // cod_fee = cod_fee.replace(/[^\d.-]/g, '');
                    ...
                    var pay_total = total + tax + fee + (0.18 * fee);

                    var rounded_pay_total = Math.round(pay_total);
                    var round_off = rounded_pay_total - pay_total;

                    cod_fee.removeClass('d-none');

                    // Format the numbers to two decimal places and include the options to ensure decimal points
                    var formattedTotal = new Intl.NumberFormat('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format((total + fee));
                    var formattedTax = new Intl.NumberFormat('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format((tax + (0.18 * fee)));
                    var formattedRoundOff = new Intl.NumberFormat('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(round_off);
                    var formattedPayTotal = new Intl.NumberFormat('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(rounded_pay_total);

                    // Update the HTML content
                    $('.sub_total').html('₹ ' + formattedTotal);
                    $('.tax').html('₹ ' + formattedTax);
                    $('.round_off').html('₹ ' + formattedRoundOff);
                    $('.pay_total').html('₹ ' + formattedPayTotal);
                    ...
                    var pay_total = total + tax;

                    var rounded_pay_total = Math.round(pay_total);
                    var round_off = rounded_pay_total - pay_total;

                    cod_fee.addClass('d-none');
                    // Format the numbers to two decimal places and include the options to ensure decimal points
                    var formattedTotal = new Intl.NumberFormat('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(total);
                    var formattedTax = new Intl.NumberFormat('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(tax);
                    var formattedRoundOff = new Intl.NumberFormat('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(round_off);
                    var formattedPayTotal = new Intl.NumberFormat('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(rounded_pay_total);
