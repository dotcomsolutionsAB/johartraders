...
if (!function_exists('cart_product_price')) {
    ...
    {
        if (isset($product->auction_product) && $product->auction_product == 0) {
            $str = '';
...
if (!function_exists('get_products_by_category')) {
    function get_products_by_category($limit, $category_id, $user_id = null)
    {
        // Retrieve the category and its child categories
        $category = Category::with('childrenCategories')->find($category_id);

        if ($category) {
            // Get all child category IDs including the parent category itself
            $categoryIds = $category->childrenCategories()->pluck('id')->push($category->id);

            // Query products from the specified categories
